...
\caption{FinRisk-OPE pipeline from public portfolio panels to full-information rewards, known-propensity logs, return/risk OPE diagnostics, and SCROPE++ selective certification.}
\label{fig:pipeline}
\end{figure}
...
\input{generated_tables/table_benchmark_summary.tex}
...
\input{generated_tables/table_ope_by_support.tex}
...
\input{generated_tables/table_risk_ope_by_family.tex}
...
\input{generated_tables/table_matched_abstention_comparison.tex}
...
Beyond the proposed benchmark and certification framework, the broader implication of this work is methodological. Financial offline learning should not be evaluated solely by estimated returns or backtest rankings. Instead, evaluation should ask whether the available counterfactual evidence is sufficient to justify deployment. By combining known propensities, exact counterfactual rewards, support diagnostics, risk estimation, and selective certification within a date-frozen public benchmark, FinRisk-OPE provides a reproducible setting for studying this question.

Several limitations remain. The logging policies are constructed rather than recovered from real investor behavior. The current evaluation omits transaction costs and market impact; rewards are based on realized portfolio-panel returns and do not model execution frictions. The benchmark also does not yet cover richer transaction-level, order-book, or product-interaction environments. Future work should incorporate validated real-world logs, explicit execution-cost and market-impact models, and richer distributional-risk estimators. More broadly, extending certification beyond expected performance toward robust guarantees under support uncertainty remains an important direction for financial decision intelligence.
Ultimately, the central lesson is simple: a policy that appears profitable is not necessarily a policy that can be trusted. Reliable financial deployment requires not only prediction and estimation, but certification of the evidence on which those decisions rest.
